...
const createTableSql = (table: TableSnapshot): SQLStatement => {
    ...
    if (table.primaryKey.length > 0) {
        statement.append`,\n  PRIMARY KEY (${sql.join(
            table.primaryKey.map(sql.ident),
            ", ",
        )})`;
    }
...
const alterColumnSql = (
    ...
    // Join multiple ALTER statements with newline
    const result = new SQLStatement("");
    statements.forEach((s, i) => {
        result.push(s);
        if (i < statements.length - 1) {
            result.push(new SQLStatement("\n"));
        }
    });
    return result;
};
...
const alterPrimaryKeySql = (op: {
    ...
    if (op.oldColumns.length > 0) {
        // We need to find the PK constraint name - use the conventional naming
        const pkName = `${op.table}_pkey`;
        statements.push(
            sql`ALTER TABLE ${sql.ident(op.schema)}.${sql.ident(op.table)} DROP CONSTRAINT IF EXISTS ${sql.ident(pkName)};`,
        );
    }
