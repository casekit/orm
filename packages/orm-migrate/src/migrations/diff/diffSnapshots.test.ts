...
const makeTable = (
    overrides: Partial<TableSnapshot> & { name: string },
): TableSnapshot => ({
    schema: "app",
    columns: [],
    primaryKey: [],
...
describe("diffSnapshots", () => {
    ...
    describe("unique constraints", () => {
        test("detects added unique constraint", () => {
            ...
            const desired: SchemaSnapshot = {
                ...
                extensions: [],
                tables: [
                    makeTable({ name: "users", uniqueConstraints: [uc] }),
                ],
            };
    ...
    describe("primary keys", () => {
        test("detects changed primary key", () => {
            const current: SchemaSnapshot = {
                ...
                extensions: [],
                tables: [
                    makeTable({ name: "users", primaryKey: ["id"] }),
                ],
            };
            const desired: SchemaSnapshot = {
                schemas: ["app"],
    ...
    describe("operation ordering", () => {
        ...
        test("drops foreign keys before dropping columns", () => {
            ...
            const ops = diffSnapshots(current, desired);
            const dropFkIdx = ops.findIndex(
                (o) => o.type === "dropForeignKey",
            );
            const dropColIdx = ops.findIndex(
                (o) => o.type === "dropColumn",
            );
        ...
        test("drops tables before schemas", () => {
            ...
            const ops = diffSnapshots(current, desired);
            const dropTableIdx = ops.findIndex(
                (o) => o.type === "dropTable",
            );
            const dropSchemaIdx = ops.findIndex(
                (o) => o.type === "dropSchema",
            );
